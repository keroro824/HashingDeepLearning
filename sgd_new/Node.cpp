...
void Node::init(LSH *hashTables, WtaHash *wtaHasher, DensifiedMinhash *MinHasher, SparseRandomProjection *srp, DensifiedWtaHash *dwtaHasher, int * binids, int L ){
	_hashTables = hashTables;
	_wtaHasher = wtaHasher;
	_MinHasher = MinHasher;
	_srp = srp;
	_dwtaHasher = dwtaHasher;
	_binids = binids;
	_L = L;
}
...
//	if (_IDinLayer == label)
		_lastDeltaforBPs[inputID] = (1.0/labelsize - _lastActivations[inputID]) ;
	}
	else {
...
void Node::backPropagate(Node** previousNodes, int* previousLayerActiveNodeIds, int previousLayerActiveNodeSize, float learningRate, int inputID, bool rehash)
{
	assert(("Input Not Active but still called !! BUG", _ActiveinputIds[inputID] == 1));
...
//		_adamAvgVelbias = _adamAvgVelbias / (1 - pow(BETA2, _tbias));
    ...
    if (rehash) {
        ...
        int *hashes;
        if (HashFunction == 1) {
            hashes = _wtaHasher->getHash(_weights);
        } else if (HashFunction == 2) {
            hashes = _dwtaHasher->getHashEasy(_weights, _dim, TOPK);
        } else if (HashFunction == 3) {
            hashes = _MinHasher->getHashEasy(_binids, _weights, _dim, TOPK);
        } else if (HashFunction == 4) {
            hashes = _srp->getHash(_weights, _dim);
        }

        int *hashIndices = _hashTables->hashesToIndex(hashes);
        int * bucketIndices = _hashTables->add(hashIndices, _IDinLayer+1);
//        for (int i = 0; i < _L; i++) {
//
...
void Node::backPropagateFirstLayer(int* nnzindices, float* nnzvalues, int nnzSize, float learningRate, int inputID, bool rehash)
{
	assert(("Input Not Active but still called !! BUG", _ActiveinputIds[inputID] == 1));
