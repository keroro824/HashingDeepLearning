...
import glob
train_files = glob.glob('/efs/users/beidchen/workspace/aloi/aloi_lsh/data/amazon-3M_shuf_train.txt')
test_files = glob.glob('/efs/users/beidchen/workspace/aloi/aloi_lsh/data/amazon-3M_shuf_test.txt')
...
with tf.device('/gpu:4'):
    W11 = tf.Variable(tf.truncated_normal([hidden_dim_1,n_classes//4], stddev=0.01))
    b11 = tf.Variable(tf.zeros([n_classes//4]))
    layer_11 = tf.matmul(layer_1,W11)+b11


with tf.device('/gpu:1'):
    W12 = tf.Variable(tf.truncated_normal([hidden_dim_1,n_classes//4], stddev=0.01))
    b12 = tf.Variable(tf.zeros([n_classes//4]))
    layer_12 = tf.matmul(layer_1,W12)+b12


with tf.device('/gpu:2'):
    W13 = tf.Variable(tf.truncated_normal([hidden_dim_1,n_classes//4], stddev=0.01))
    b13 = tf.Variable(tf.zeros([n_classes//4]))
    layer_13 = tf.matmul(layer_1,W13)+b13


with tf.device('/gpu:3'):
    W14 = tf.Variable(tf.truncated_normal([hidden_dim_1,n_classes//4+n_classes%4], stddev=0.01))
    b14 = tf.Variable(tf.zeros([n_classes//4+n_classes%4]))
    layer_14 = tf.matmul(layer_1,W14)+b14
    logits = tf.concat([layer_11, layer_12, layer_13, layer_14], axis=-1)
...
# W2 = tf.Variable(tf.truncated_normal([hidden_dim_1,n_classes], stddev=2.0/math.sqrt(hidden_dim_2+n_classes)))
# b2 = tf.Variable(tf.truncated_normal([n_classes], stddev=2.0/math.sqrt(n_classes+hidden_dim_2)))
