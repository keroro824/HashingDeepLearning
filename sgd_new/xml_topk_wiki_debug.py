...
params = np.load('/search_labs/users/beidchen/workspace/SUBLIME/HashingDeepLearning/bgd_new/weights_wiki.npz')
...
# np.savez_compressed('/search_labs/users/beidchen/workspace/SUBLIME/HashingDeepLearning/bgd_new/weights_wiki.npz', weight_0=sess.run(W1), bias_0=sess.run(b1), weight_1=sess.run(W2), bias_1=sess.run(b2))
...
for i in range(n_steps):
    ...
    if i==0:
        ...
        np.savetxt("/search_labs/users/beidchen/workspace/SUBLIME/HashingDeepLearning/bgd_new/weights3", actives.astype(int), fmt='%d', delimiter="")
