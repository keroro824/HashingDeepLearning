...
import glob
train_files = glob.glob('/efs/users/beidchen/workspace/aloi/aloi_lsh/data/WikipediaLarge-500K_shuf_train.txt')
test_files = glob.glob('/efs/users/beidchen/workspace/aloi/aloi_lsh/data/WikipediaLarge-500K_shuf_test.txt')
...
################### Top-k
#
W2 = tf.Variable(tf.truncated_normal([n_classes,hidden_dim_1], stddev=0.01))
b2 = tf.Variable(tf.zeros([n_classes]))
#
logits = tf.matmul(layer_1, tf.transpose(W2))
